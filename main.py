...
def files_management():
    for (repertoire, sousRepertoires, fichiers) in walk(path_film):
...
def download():
    for n in range(len(listeFichiers)):
        if not (not ('.mp4' in listeFichiers[n]) and not ('.avi' in listeFichiers[n]) and not (
            ...
            selection = str(listeFichiers[n][0:20])
...
class Application(tk.Tk):
    ...
    def creer_widgets(self):
        ...
        self.Button_start = tk.Button(self, command=start, image=self.Start_image, borderwidth=0, bg='WHITE',
